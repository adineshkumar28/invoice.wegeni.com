 ...
 */
class ClientQuoteRepository extends BaseRepository
{
    /**
     * @var string[]
     */
    ...
    public function getQuoteItemList(array $quote = []): array
    {
        $quoteItems = [];
    ...
    public function getSyncList(array $quote = []): array
    {
        ...
        $data['template'] = InvoiceSetting::withoutGlobalScope(new TenantScope())->pluck('template_name', 'id')->toArray();
    ...
    public function saveQuote(array $input): Quote
    {
        try {
            ...
            /** @var Quote $quote */
            $input['client_id'] = Client::withoutGlobalScope(new TenantScope())->whereUserId($input['client_id'])->first()->id;
    ...
    public function updateQuote($quoteId, $input)
    {
        try {
            ...
            if ($input['client_id'] != Auth::id()) {
                throw new UnprocessableEntityHttpException('Quote can\'t be edited.');
            }
            /** @var Quote $quote */
