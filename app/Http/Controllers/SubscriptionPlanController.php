...
class SubscriptionPlanController extends AppBaseController
{
    ...
    {
        $result = Subscription::where('subscription_plan_id', $subscriptionPlan->id)->where(
            'status',
            Subscription::ACTIVE
        )->count();
        if ($result > 0) {
    ...
    {
        $defaultSubscriptionPlan = SubscriptionPlan::where('is_default', 1)->update(['is_default' => 0]);
        // $defaultSubscriptionPlan->update(['is_default' => 0]);
        SubscriptionPlan::where('is_default', 1)->update(['is_default' => 0]);
