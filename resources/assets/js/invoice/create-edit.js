...
document.addEventListener("DOMContentLoaded", loadCreateEditInvoice);
...
listenChange("#discountType", function () {
    ...
    if (discountType == 1 || discountType == 2) {
        $("#discount").removeAttr("disabled");
        $('#invoicesDiscounBeforeTax').attr("disabled", "disabled");
        if (discountType == 2) {
            $('#invoicesDiscounBeforeTax').removeAttr('disabled');
...
listenChange("#invoicesDiscounBeforeTax", function () {
    calculateFinalAmount();
});
...
listenClick("#addItem", function () {
    ...
    $(".productId").select2({
        placeholder: Lang.get("js.select_product"),
        tags: true,
    });
...
const calculateFinalAmount = () => {
    ...
    let isDiscountBeforeTax = $("#invoicesDiscounBeforeTax").is(":checked");
...
listenClick("#saveAsDraft,#saveAndSend", function (event) {
    ...
    let discountBeforeTax = $("#invoicesDiscounBeforeTax").prop("checked") ? 1 : 0;
    ...
    if (invoiceStates == 1) {
        swal({
            title:
                ...
                " !",
            text:
                Lang.get("js.Are_you_sure_to_send_invoice") +
                " ?",
            icon: "warning",
            buttons: {
                confirm:
...
listenClick("#editSaveAndSend,#editSave", function (event) {
    ...
    let formData =
        ...
        "&tax=" +
        JSON.stringify(tax) + "&discount_before_tax=" + discountBeforeTax;
    if (invoiceStatus == 1) {
        swal({
            title:
                Lang.get("js.send") +
