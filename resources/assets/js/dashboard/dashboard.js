...
function prepareYearlyIncomeViewChart(result) {
    ...
    let yearlyIncomeChart = document.getElementById(
        "yearly_income_chart_canvas"
    );
    ...
    let ctx = yearlyIncomeChart.getContext("2d");
...
function prepareInvoiceViewStatusChart(result) {
    ...
    let invoiceOverview = document.getElementById("invoice_overview");

    if (invoiceOverview === null) {
        return false;
    }

    let ctx = invoiceOverview.getContext("2d");
    let pieChartData = {
        labels: data.labels,
        datasets: [
            {
...
function preparePaymentViewStatusChart(result) {
    ...
    let paymentOverview = document.getElementById("payment_overview");

    if (paymentOverview === null) {
        return false;
    }

    let ctx = paymentOverview.getContext("2d");
    let pieChartData = {
        labels: data.labels,
        datasets: [
            {
