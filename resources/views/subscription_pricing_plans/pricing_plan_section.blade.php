        ...
        <div class="card-body px-10 py-14">
                ...
                <div class="justify-content-center mt-5">
                    <h4 class="text-center mb-6 mt-2">
                        ...
                        <span class="h6 text-gray-800 ml-2">/
                            @if ($subscriptionsPricingPlan->frequency == \App\Models\SubscriptionPlan::MONTH)
                                {{ __('messages.admin_dashboard.month') }}
                            @elseif ($subscriptionsPricingPlan->frequency == \App\Models\SubscriptionPlan::YEAR)
                                {{ __('messages.admin_dashboard.year') }}
                            @elseif ($subscriptionsPricingPlan->frequency == \App\Models\SubscriptionPlan::UNLIMITED)
                ...
                <div class="pricing-card__data">
                    <ul class="pl-0 list-style-none">
                        @if (isAuth() && count($subscriptionsPricingPlan->subscription) > 0)
                            ...
                            @if (
                                $activeSubscription &&
                                    $activeSubscription->trial_ends_at != null &&
                                    $activeSubscription->subscription_plan_id == $subscriptionsPricingPlan->id)
                                <li>
            ...
            @if (
                $currentActiveSubscription &&
                    $subscriptionsPricingPlan->id == $currentActiveSubscription->subscription_plan_id &&
            ...
            @else
                @if (
                    $currentActiveSubscription &&
                        !$currentActiveSubscription->isExpired() &&
                        ($subscriptionsPricingPlan->price == 0 || $subscriptionsPricingPlan->price != 0))
