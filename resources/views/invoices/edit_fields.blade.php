{{-- new partial for invoice edit: adds Client Group + Client selects, prepopulates old data, and matches JS IDs --}}
<div class="row">
  <div class="col-lg-6">
    <div class="mb-5">
      ...
      {{ Form::select(
          'client_group_id',
          $clientGroups ?? [],
          old('client_group_id', $invoice->client->client_group_id ?? null),
          [
            'id' => 'clientGroupId',
            'class' => 'form-select form-select-solid',
            'placeholder' => __('messages.client.select_client_group'),
  ...
  <div class="col-lg-6">
    <div class="mb-5">
      ...
      {{-- ensure selected value uses user_id (not client_id) and options are provided --}}
      {{ Form::select(
          'client_id',
          $clients ?? [],
          old('client_id', $invoice->client->user_id ?? null),
          [
            'id' => 'clientId',
            'class' => 'form-select form-select-solid io-select2',
            'placeholder' => __('messages.invoice.select_client'),
            'data-control' => 'select2',
          ]
      ) }}
    </div>
...
<div class="row">
  <div class="col-lg-4 col-sm-12 mb-5">
    {{-- invoice number with optional prefix/suffix --}}
    @if (!empty(getSettingValue('invoice_no_prefix')) || !empty(getSettingValue('invoice_no_suffix')))
      <div class="" data-bs-toggle="tooltip" data-bs-trigger="hover" title="invoice number">
        <div class="form-group col-sm-12 mb-5">
          ...
          <div class="input-group">
            @if (!empty(getSettingValue('invoice_no_prefix')))
              <a class="input-group-text bg-secondary border-0 text-decoration-none text-black" title="Invoice No Prefix">
            ...
            @if (!empty(getSettingValue('invoice_no_suffix')))
              <a class="input-group-text bg-secondary border-0 text-decoration-none text-black" title="Invoice No Suffix">
    ...
    @else
      <div class="" data-bs-toggle="tooltip" data-bs-trigger="hover" title="{{ __('Invoice Number') }}">
...
<div class="col-12 text-end my-5">
  <button type="button" class="btn btn-primary text-start" id="addItem">{{ __('Add Insurance') }}</button>
</div>
...
<div class="float-end">
  <div class="form-group col-sm-12">
    {{-- Keep your existing buttons/flow (normal PUT submission). If you later want AJAX update, we can enable a #updateInvoice button. --}}
    <button type="submit" name="save" class="btn btn-primary mx-1 ms-ms-3 mb-3 mb-sm-0">{{ __('Update & Send') }}</button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $('#addItem').on('click', function () {
        let $container = $('.invoice-item-container');
        let $lastRow = $container.find('tr:last');

        // clone panna munnaadi select2 destroy pannunga
        $lastRow.find('.io-select2').select2('destroy');

        let $newRow = $lastRow.clone();

        // Reset inputs
        $newRow.find('select.insurance').val('').trigger('change');
        $newRow.find('input.policy-number').val('');
        $newRow.find('input.qty').val(1);
        $newRow.find('input.price').val(0);
        $newRow.find('select.tax').val(null).trigger('change');
        $newRow.find('.amount-value').text('0.00');

        // Update row number
        let newIndex = $container.find('tr').length + 1;
        $newRow.find('.item-number').text(newIndex);

        // Append row
        $container.append($newRow);

        // Re-init select2 for old and new rows
        $container.find('.io-select2').select2({
            width: '100%'
        });
    });
    ...
    $(document).on('click', '.delete-invoice-item', function () {
        ...
        if ($container.find('tr').length > 1) {
            ...
            // Re-number rows
            $container.find('.item-number').each(function (i) {
                $(this).text(i + 1);
            });
        }
    });
