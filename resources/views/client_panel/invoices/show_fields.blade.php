                                ...
                                <div
                                    ...
                                    <div class="mb-8">
                                        @if ($invoice->status == \App\Models\Invoice::UNPAID)
                                            <span
                                                class="badge bg-light-danger">{{ __('messages.pending_payment') }}</span>
                                        @elseif($invoice->status == \App\Models\Invoice::PAID)
                                            <span class="badge bg-light-success me-2">{{ __('messages.paid') }}</span>
                                        @elseif($invoice->status == \App\Models\Invoice::PARTIALLY)
                                            <span
                                                class="badge bg-light-primary">{{ __('messages.partially paid') }}</span>
                                        ...
                                        @elseif($invoice->status == \App\Models\Invoice::OVERDUE)
                                            <span class="badge bg-light-danger">{{ __('messages.overdue') }}</span>
                                        @elseif($invoice->status == \App\Models\Invoice::PROCESSING)
                                            <span
                                                class="badge bg-light-primary">{{ __('messages.processing') }}</span>
                                        @endif
                                    </div>
                                        ...
                                        @php
                                            ...
                                            if ($invoice->status != \App\Models\Invoice::PAID) {
                                                foreach ($invoice->payments as $payment) {
                                                    if (
                                                        $payment->payment_mode == \App\Models\Payment::MANUAL &&
                                                        $payment->is_approved !== \App\Models\Payment::APPROVED
                                                    ) {
                                                        continue;
                                                    }
                                                    $paid += $payment->amount;
