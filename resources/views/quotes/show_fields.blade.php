                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-3 col-md-3 col-6">
                                        <div class="d-flex flex-column mb-md-10 mb-5 mt-3 mt-md-0">
                                            ...
                                            <span
                                                class="fs-4 text-gray-800">{{ \Carbon\Carbon::parse($quote->quote_date)->translatedFormat(currentDateFormat()) }}</span>
                                        </div>
                                    </div>
                                    <div class="col-lg-3 col-md-3 col-6">
                                        <div class="d-flex flex-column mb-md-10 mb-5 mt-3 mt-md-0">
                                            ...
                                            <span
                                                class="fs-4 text-gray-800">{{ \Carbon\Carbon::parse($quote->due_date)->translatedFormat(currentDateFormat()) }}</span>
                                        </div>
                                    </div>
                                    <div class="col-md-2 col-6 mb-3 mb-sm-0">
                                        @if ($isPublicView)
                                            <a target="_blank" href="{{ route('quotes.pdf', ['quote' => $quote->id]) }}"
                                    ...
                                    <div class="col-lg-5 col-md-6">
                                        <div class="d-flex flex-column mb-md-10 mb-5">
                                            ...
                                            <span
                                                class="fs-4 text-gray-800 mb-3">{{ $quote->client->user->full_name }}</span>
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th scope="col" class="text-end">
                                                        {{ __('messages.quote.price') }}</th>
                                                    <th scope="col" class="text-end">
                                                        {{ __('messages.invoice.tax') . ' (in %)' }}
                                                    </th>
                                                    <th scope="col" class="text-end">
                                                        {{ __('messages.quote.amount') }}</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach ($quote->quoteItems as $index => $quoteItem)
                                                    <tr>
                                                        <td class="py-4">
                                                            {{ isset($quoteItem->product->name) ? $quoteItem->product->name : $quoteItem->product_name ?? __('messages.common.n/a') }}
                                                        </td>
                                                        <td class="py-4">{{ $quoteItem->quantity }}</td>
                                                        <td class="py-4 text-end min-width-130px">
                                                            {{ isset($quoteItem->price) ? getCurrencyAmount($quoteItem->price, true, $quote->tenant_id) : __('messages.common.n/a') }}
                                                        </td>
                                                        ...
                                                        <td class="py-4 text-end min-width-130px">
                                                            {{ isset($quoteItem->total) ? getCurrencyAmount($quoteItem->total, true, $quote->tenant_id) : __('messages.common.n/a') }}
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                    ...
                                    @if (count($quote->qouteTaxes) > 0)
                                        <div class="col-lg-7">
                                            ...
                                            <div class="mt-2">
                                                @foreach ($quote->qouteTaxes as $tax)
                                                    <div class="mb-1">
                                    ...
                                    <div class="col-lg-5 ms-lg-auto mt-4">
                                        <div class="border-top">
                                            <table class="table table-borderless box-shadow-none mb-0 mt-5">
                                                <tbody>
                                                    <tr>
                                                        <td class="ps-0">{{ __('messages.quote.sub_total') . ':' }}
                                                        </td>
                                                        <td class="text-gray-900 text-end pe-0">
                                                            {{ isset($quote->amount) ? getCurrencyAmount($quote->amount, true, $quote->tenant_id) : __('messages.common.n/a') }}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td class="ps-0">{{ __('messages.quote.discount') . ':' }}
                                                        </td>
                                                        <td class="text-gray-900 text-end pe-0">
                                                            ...
                                                            @else
                                                                ...
                                                                @else
                                                                    @if ($quote->discount_before_tax)
                                                                        {{ getCurrencyAmount(($quote->amount * $quote->discount) / 100, true, $quote->tenant_id) }}
                                                                    @else
                                                                        @php
                                                                            $itemTaxesAmount =
                                                                                $quote->amount + array_sum($totalTax);
                                                                            $quoteTaxesAmount =
                                                                                ($itemTaxesAmount *
                                                                                    $quote->qouteTaxes->sum('value')) /
                                                                                100;
                                                                            $percentageDiscount =
                                                                                $itemTaxesAmount + $quoteTaxesAmount;
                                                                        @endphp

                                                                        {{ getCurrencyAmount(($percentageDiscount * $quote->discount) / 100, true, $quote->tenant_id) }}
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-gray-900 text-end pe-0">
                                                            {!! numberFormat($totalTaxes) != 0
                                                                ? getCurrencyAmount($totalTaxes, true, $quote->tenant_id)
                                                                : __('messages.common.n/a') !!}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                ...
                                <div
                                    ...
                                    <div class="mb-8">
                                        @if ($quote->status == \App\Models\Quote::DRAFT)
                                            <span
                                                class="badge bg-light-warning me-5">{{ __('messages.draft') }}</span>
                                        @elseif($quote->status == \App\Models\Quote::CONVERTED)
                                            <span
                                                class="badge bg-light-success me-5">{{ __('messages.converted') }}</span>
                                        @endif
                                    </div>
