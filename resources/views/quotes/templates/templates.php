<script id="quotesItemTemplate" type="text/x-jsrender">
    ...
    {{if clientQuote}}
        <td class="table__item-desc w-25">
            <select class="form-select productId client-product-quote form-select-solid fw-bold" name="product_id[]" data-control="select2" required>
        ...
        {{/if}}
    <td class="table__qty">
        <input class="form-control qty-quote form-control-solid" required="" name="quantity[]" type="number" min="0" step=".01" oninput = "this.value = this.value.replace(/\./g, '').replace(/\D/g, '')">
    </td>
    <td>
    ...
    <td class="">
        <select data-link="defaultTax" class="form-select taxIdQuote taxQuote form-select-solid fw-bold" name="tax[]" placeholder="Select Tax" multiple="multiple">
            {{for taxes}}
               <option value="{{:value}}" data-id="{{:id}}">{{:name}}</option>
            {{/for}}
        </select>
    </td>
